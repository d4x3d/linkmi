...
export const handleCallback = action({
  ...
  handler: async (ctx, args) => {
    try {
      ...
      const metadata =
        typeof transactionData.metadata === 'string'
      ...
      try {
        const productInfo = await ctx.runQuery(internal.products.getDownloadInfo, { id: productId });
        if (productInfo) {
          await ctx.runAction(internal.email.sendPurchaseReceipt, {
            email: customerEmail,
            productName: productInfo.name,
            downloadUrl: productInfo.downloadUrl || undefined,
            deliveryNote: productInfo.deliveryNote || undefined,
          });
          console.log('Receipt email sent');
        }
      } catch (err) {
        console.error('Failed to send receipt email:', err);
        // Continue execution, don't revert purchase
      }
